...
exports.getAppointments=async (req,res,next)=>{
    ...
    try{
        ...
        console.log(err);
        return res.status(500).json({sucess:false,message:"Cannot find Appointment"});
    }
};
...
exports.updateAppointment= async (req,res,next) => {
    try{
        ...
        if(!appointment){
            return res.status(404).json({success:false,message:`No appointmant with the id of ${req.params.id}`});
        }
...
exports.deleteAppointment= async (req,res,next) => {
    try{
        ...
        //Make sure is the appointmentment owner
        if(appointment.user.toString() !== req.user.id && req.user.role !== 'admin'){
